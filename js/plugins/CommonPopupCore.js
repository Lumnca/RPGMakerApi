...
/*:
 * @plugindesc  可以在事件和玩家上方弹出文字信息
 * @author  author Lumnca
 * 
 ...
 * 
 * 只能指定所需的参数。
 * 示例：在玩家上面弹出测试和显示 240帧
 * CommonPopup add text:测试 count:240 eventId:-1
 * 
 *参数详情：
 * text：显示文本
 * eventId：要显示的事件的ID
 ...
 * sx：显示位置校正X
 * sy：显示位置校正Y
 * pattern：显示模式0为淡出，-1为横向拉伸，-2为纵向拉伸
 * back：-1：透明背景，0：用背景颜色填充，1以上：图像索引
 * bx：内容的显示位置校正X
 * by：内容的显示位置校正Y
 * extend：通过显示定时的调整用排列指定。例：extend:[20,50]乘以20帧出现，从第50帧开始消失。
 * fixed：固定在画面上吗？指定为true/false。
 * anchorX:
 * anchorY:
 * slideCount：发生新弹出时，向上滑动的速度。
 *
 *使用事件命令的脚本时
 *
...
(function(){
    ...
    CommonPopupManager.window = function(fs){
        if(fs){
            this._window.contents.fontSize = fs;
        }
    ...
    Sprite_Popup.prototype.createBitmap = function(){
        if (this._arg.bitmap){
            ...
            CommonPopupManager.window(14).drawTextEx(this._arg.text,12,4);
