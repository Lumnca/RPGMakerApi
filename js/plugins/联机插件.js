 ...
 * 
 * Version 1.2:解决了角色进入战斗场景和菜单场景显示出错问题
 *
 */
...
function subscribe() {
    stompClient.subscribe("/topic/new_add", function (res) {
        ...
        if (data.id !== player.id && player.mapId === $gameMap._mapId && isGameMap()) {
            addNewEvent(player);
        }
...
function getPlayerData() {
    ...
    data.scene = SceneManager._scene._sceneName;
...
function addNewEvent(data) {
    ...
    if (player) {
        ...
        if( $gameMap._events.indexOf(player.event)===-1){
...
Game_Event.prototype.update = function () {
    if (this._isPlayer) {
        if ((this.dx !== this.x || this.dy !== this.y) && new Date().getTime()-this._timeCount > 250) {
            this._timeCount = new Date().getTime();
...
Scene_Map.prototype.terminate = function() {
    ...
    data.scene = SceneManager._nextScene._sceneName;
    if( data.scene==='map'){
...
const scene_initialize = Scene_Map.prototype.initialize;
Scene_Map.prototype.initialize = function(){
    scene_initialize.call(this);
    this._sceneName = 'map';
}
const scene_menuNase_initializ = Scene_MenuBase.prototype.initialize;
Scene_MenuBase.prototype.initialize = function(){
    scene_menuNase_initializ.call(this);
    this._sceneName = 'menu';
}
...
//===============================================================================================
var sk = 0;
const s = Scene_Battle.prototype.startActorCommandSelection;
