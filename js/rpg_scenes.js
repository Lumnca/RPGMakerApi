 ...
 * 创建组件并将其添加到渲染过程中。先于start在场景渲染之前执行
 * @method create
 * @instance 
 * @memberof Scene_Base
 */
 ...
 * 将子窗口添加到windowLayer处理中。即为场景图层添加子组件
 * @method addWindow
 * @instance 
 * @memberof Scene_Base
 */
 ...
 * 请求淡入屏幕进程。即场景淡进特效实现
 * @method startFadeIn
 * @param {Number} [duration=30] The time the process will take for fadeIn the screen 该过程在屏幕上淡入淡出所需的时间（ms）
 * @param {Boolean} [white=false] If true the fadein will be process with a white color else it's will be black
 *  如果是true，fadein将是白色的，否则它将是黑色的
 * 
 * @instance 
 * @memberof Scene_Base
 */
 ...
 * 为fadein和fadeOut创建一个屏幕精灵，并将其添加到渲染过程中。
...
Scene_Boot.prototype.create = function() {
    ...
    DataManager.loadDatabase();//加载图片，数据文件
    ConfigManager.load();
...
Scene_Boot.prototype.start = function() {
    ...
    if (DataManager.isBattleTest()) {
        DataManager.setupBattleTest();//战斗测试界面
        SceneManager.goto(Scene_Battle);
    } else if (DataManager.isEventTest()) {
        DataManager.setupEventTest();
        SceneManager.goto(Scene_Map);//传到地图开始界面
    } else {
        this.checkPlayerLocation();
        DataManager.setupNewGame();
        SceneManager.goto(Scene_Title);//游戏开始标题界面
